...
class Experiments:
    ...
    def play_in_threads(self, first_player, second_player):
        score = {'first_player': {'player_type': first_player.get_name(), 'score': 0}, 
                 'second_player': {'player_type': second_player.get_name(), 'score': 0}}
        ...
        for _ in range(self.iterations):
            thread = threading.Thread(target=self.play_single_game, args=(first_player, second_player, score_mutex, score))
            thread.start()
    ...
    def run_experiments(self):
        ...
        self.players = [MctsPlayer(mcts_config), Ucb1TunedPlayer(mcts_config), MinimaxPlayer(minimax_config)]
